...
public class CalendarioDao {
	...
	public boolean guardar(TransferCalendario calendario) throws SQLException {
		String sql = """
				...
				(vuelo_id, fecha_inicio, fecha_fin, hora_salida, hora_llegada)
				VALUES (?, ?, ?, ?, ?)
				""";
	...
	public List<TransferCalendario> buscarPorVueloId(String vueloId) throws SQLException {
		String sql = """
				SELECT vuelo_id, fecha_inicio, fecha_fin, hora_salida, hora_llegada
				FROM calendario
				WHERE vuelo_id = ?
	...
	private TransferCalendario mapearCalendario(ResultSet rs) throws SQLException {
		return new TransferCalendario(rs.getDate("fecha_inicio").toLocalDate(), rs.getDate("fecha_fin").toLocalDate(),
				rs.getTime("hora_salida").toLocalTime(), rs.getTime("hora_llegada").toLocalTime(),
