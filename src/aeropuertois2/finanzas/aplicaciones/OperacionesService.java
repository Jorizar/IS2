...
public class OperacionesService {
    ...
    public void realizarTransferencia(String ibanOrigen, String ibanDestino, double monto, String concepto) 
        ...
        if (monto <= 0) throw new ValidationException("El monto a transferir debe ser mayor a 0.");
        if (ibanOrigen.equals(ibanDestino)) throw new ValidationException("La cuenta de origen y destino no pueden ser la misma.");
        ...
        if (cuentaOrigen == null) throw new ValidationException("La cuenta de origen no existe.");
        if (cuentaDestino == null) throw new ValidationException("La cuenta de destino no existe.");
        if (!"VALIDADA".equals(cuentaOrigen.getEstadoValidacion())) throw new ValidationException("La cuenta de origen no está validada.");
        if (cuentaOrigen.getSaldo() < monto) throw new ValidationException("Saldo insuficiente en la cuenta de origen.");
        ...
        contabilidadService.crearRegistro(ibanOrigen, concepto + " (Hacia: " + ibanDestino + ")", "TRANSFERENCIA", monto);
    }
}