...
public class ContabilidadService {
    ...
    public void crearRegistro(String cuenta, String concepto, String tipo, double monto) throws ValidationException, SQLException {
    ...
    public void modificarRegistro(String idRegistro, String cuenta, String concepto, String tipo, double monto) throws ValidationException, SQLException {
        if (idRegistro == null || idRegistro.trim().isEmpty()) throw new ValidationException("El ID del registro es obligatorio.");
        validarDatos(cuenta, concepto, monto);
        validarDebeHaber(tipo, monto);
    ...
    private void validarDatos(String cuenta, String concepto, double monto) throws ValidationException {
        if (cuenta == null || cuenta.trim().isEmpty()) throw new ValidationException("La cuenta contable no puede estar vacía.");
        if (concepto == null || concepto.trim().isEmpty()) throw new ValidationException("El concepto no puede estar vacío.");
    ...
    private void validarDebeHaber(String tipo, double monto) throws ValidationException {
        if (tipo.equals("EGRESO") && monto > 100000) {
            throw new ValidationException("Desbalance detectado: El egreso supera el límite permitido sin autorización superior.");
        }
    }
