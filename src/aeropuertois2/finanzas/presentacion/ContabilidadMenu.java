	...
	private void ejecutarVerRegistros() {
		...
		try {
			...
			if (registros.isEmpty()) {
				...
				System.out.printf("%-12s | %-26s | %-15s | %-10s | %-10s%n", "ID", "CUENTA", "TIPO", "MONTO (€)",
						"BALANCE");
				System.out
						.println("----------------------------------------------------------------------------------");
				for (RegistroContable r : registros) {
					System.out.printf("%-12s | %-26s | %-15s | %-10.2f | %-10s%n", r.getIdRegistro(),
							r.getCuentaContable(), r.getTipoOperacion(), r.getMonto(),
